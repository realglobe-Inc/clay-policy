...
const { requiredFieldNames, formatFields, validateFields } = restrictionHelper
const {
  isMissing,
  isTypeOf,
  isEnumOf,
...
class ClayPolicy {
  ...
  constructor (properties = {}) {
    const s = this
    s.$$fields = formatFields(properties)
  ...
  validate (entity) {
    const s = this
    const { $$fields } = s
    let missing = requiredFieldNames($$fields).filter((name) => isMissing(entity[ name ]))
    let names = Object.keys(entity)
    let knownNames = names.filter((name) => !!$$fields[ name ])
