...
class ClayPolicy {
   ...
   * @param {string} [options.prefix] - Error prefix
  ...
  validate (entity, options = {}) {
    ...
    let { prefix = null, ignoreMissing = false } = options
    ...
    let isValid = (ignoreMissing || (missing.length === 0)) && (Object.keys(failures).length === 0)
    if (isValid) {
      return null
    }
    let fieldNames = [].concat(Object.keys(failures), missing).filter(Boolean)
    let message = `Validation failed on field: ${JSON.stringify(fieldNames)}`
    if (prefix) {
      message = `[${prefix}] ${message}`
    }
    let error = new PolicyError(message, {
      missing,
      failures
    })
    delete error.stack
  ...
  validateToThrow (entity, options = {}) {
    const s = this
    let error = s.validate(entity, options)
    if (error) {
      throw error
    }
  }
  ...
  uniqueFilters (entity) {
    const s = this
    const { $$restrictions } = s
    return uniqueFieldNames($$restrictions)
      .filter((name) => !isEmpty(entity[ name ]))
      .map((name) => ({
        [name]: entity[ name ]
      }))
  }
