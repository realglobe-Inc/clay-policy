...
const { requiredFields } = restrictionHelper
const {
  isMissing,
  isTypeOf,
  isEnumOf,
...
class ClayPolicy {
  ...
  constructor (properties = {}) {
    const s = this
    s.$$fields = properties.$$fields || properties
  }
  ...
  validate (entity) {
    const s = this
    const { $$fields } = s
    let missing = requiredFields($$fields).filter((name) => isMissing(entity[ name ]))
    let failures = Object.keys(entity)
      .map((name) => {
        let failure = s.testRestriction($$fields[ name ], entity[ name ])
  ...
  testRestriction (restriction, value) {
    ...
    if (!isNaN(valueAsNumber)) {
      let rangeFail = !inRange([ minimum, maximum ], valueAsNumber)
      if (rangeFail) {
